...
def allowed_file(filename):
    ...
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in {'txt', 'png', 'jpg', 'jpeg'}
...
def hide_message():
    ...
    try:
        ...
        # Determine output filename (force PNG for lossless)
        base_name = os.path.splitext(filename)[0]
        output_filename = f'hidden_{base_name}.png'
        output_path = os.path.join(app.config['UPLOAD_FOLDER'], output_filename)
        
        result_path = crypto.hide_message(file_path, message, output_path)
